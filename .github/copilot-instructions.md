# Copilot instructions (VideoStreamPlayer)
...
- UDP datagram -> `RvfUdpReceiver` parses RVF header/payload -> raises `OnChunk` -> `RvfReassembler.Push()` copies lines into a 320×80 frame -> when `EndFrame` emits `OnFrameReady(outFrame, meta)`.
...
- When no UDP frames have arrived yet, A uses the fallback loaded/generated PGM (`GetASourceBytes()` in `MainWindow.xaml.cs`).

## RVF/UDP protocol assumptions (don’t break these)
- Header magic is ASCII `RVFU` and may be offset (receiver searches within first 64 bytes); fields are **little-endian** (see `RvfUdpReceiver`).
...
- App is `WinExe` (no console). Runtime diagnostics are written to files:
  - `udp_rx.log` for RX/datagram parsing traces (written by `MainWindow` and `RvfUdpReceiver`).
...
- Run: `dotnet run` (or `dotnet run --project VideoStreamPlayer.csproj`).
- No test project is present; changes are typically validated by running the app and observing A/B/DIFF + `udp_rx.log`.
...
- Preserve protocol compatibility in `RvfProtocol.cs`, `RvfUdpReceiver.cs`, and `RvfReassembler.cs`.
